...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision "file", source: "./EstopinalGui.pem", destination: "~/.ssh/"
  ...
  config.vm.provision "file", source: "./producer.py", destination: "~/producer.py"
  config.vm.provision "file", source: "./consumer.py", destination: "~/consumer.py"
  config.vm.provision "file", source: "./server1.properties", destination: "~/server1.properties"
  config.vm.provision "file", source: "./server2.properties", destination: "~/server2.properties"
  ...
  $script = <<-SCRIPT
     chmod go-rwx ~/.ssh/EstopinalGui.pem
     sudo apt update
     sudo apt upgrade
     ...
     python3 -m pip install --upgrade pip setuptools wheel kafka-python
  ...
  config.vm.provision "ansible_local" do |ansible|
    #ansible.playbook = "tasks/cleanup_playbook.yml"
